...
async function bump() {
  ...
  console.log(`Bumped Here! #-#-# Channel ID: ${channel.id}`);
  channel.sendTyping();
...
function startBumpTimer() {
  ...
  console.log(`Node started on username: ${client.user.username}`);
  const channel = client.channels.cache.get(process.env.CHANNELID);
  console.log(`Looking in channel: ${channel.name} of server: ${channel.guild.name}`);
...
client.on('message', message => {
  if (message.channel.id === process.env.CHANNELID && message.type === 'APPLICATION_COMMAND' && message.author.id === process.env.DISBOARD_BOT_ID && message.interaction.commandName === 'bump' && message.interaction.user.id != process.env.YourUserID) {
    console.log(`Detected Bump! Resetting.. Channel ID: ${message.channel.id}`);
